...
void DrawEllipse(BMP& newObj, long x_central, long y_central, long rad_x, long rad_y){
  ...
  x = 0, y = rad_y;
    ...
    quad1 = (rad_y * rad_y) - (rad_x * rad_x * rad_y) + (0.25 * rad_x * rad_x);
    stop_x = 2 * rad_y * rad_y * x;
    stop_y = 2 * rad_x * rad_x * y;
   ...
   quad2 = ((rad_y * rad_y) * ((x + 0.5) * (x + 0.5))) +
          ((rad_x * rad_x) * ((y - 1) * (y - 1))) -(rad_x * rad_x * rad_y * rad_y);
